    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var page in pages)
            {
                ...
                pageXml = pageXml.Replace("{inputMethod}", page.request.method);
                pageXml = pageXml.Replace("{inputAddressUrl}", page.request.url.raw.Replace("&","&amp;"));

                // If the request has headers, create and embed the header collection in Page Xml
                if (page.request.header.Count > 0)
                {
                    StringBuilder headerSb = new StringBuilder();
                    headerSb.Append("<collectioninfo>");
